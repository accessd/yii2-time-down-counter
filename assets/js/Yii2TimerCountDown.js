var timeDownCounter = function(options, whenDone) {
    ...
    var countDownReturnData;
    var countDownResSperator = '<span class="timeDownSperator">:</span>';
    var addSpanForResult = !1;
    var addSpanForEachNum = !1;
    ...
    return {
        startCountDown: function() {
            var start = setInterval(function() {
                var distance = Cookies.get(countDownIdSelector);
                var timerData = [];
                timerData[0] = get2D(Math.floor(distance / (60 * 60 * 24)));
                timerData[1] = get2D(Math.floor((distance % (60 * 60 * 24)) / (60 * 60)));
                timerData[2] = get2D(Math.floor((distance % (60 * 60)) / (60)));
                timerData[3] = get2D(Math.floor((distance % (60))));
                if (addSpanForEachNum) {
                    for (var i = 0; i < timerData.length; i++) {
                        var tempTimerData = timerData[i].toString().split('');
                        tempTimerData = tempTimerData.map(function(a, i) {
                            return "<span class='inner-item-counter-down inner-item-counter-down-" + i + "'>" + a + "</span>"
                        });
                        timerData[i] = tempTimerData.join('')
                    }
                }
                if (addSpanForResult) {
                    timerData = timerData.map(function(a, i) {
                        return "<span class='item-counter-down item-counter-down-" + i + "'>" + a + "</span>"
                    })
                }
                ...
                Cookies.set(countDownIdSelector, distance - 1);
            }, 1000)
        },
    }
}
